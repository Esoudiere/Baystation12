/*
	...
	These procs generalize various processes used by atmos machinery, such as pumping, filting, or scrubbing gas, allowing them to be reused elsewhere.
	If no gas was moved/pumped/filted/whatever, they return a negative number.
...
//Selectively moves specified gasses one gas_mixture to another and returns the amount of power needed (assuming 1 second), or -1 if no gas was filted.
//filting - A list of gasids to be scrubbed from source
...
/proc/scrub_gas(var/obj/machinery/M, var/list/filting, var/datum/gas_mixture/source, var/datum/gas_mixture/sink, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1

	filting = filting & source.gas	//only filt gasses that are actually there. DO NOT USE &=

	//Determine the specific power of each filtable gas type, and the total amount of filtable gas (gasses selected to be scrubbed)
	var/total_filtable_moles = 0			//the total amount of filtable gas
	var/list/specific_power_gas = list()	//the power required to remove one mole of pure gas, for each gas type
	for (var/g in filting)
		if (source.gas[g] < MINIMUM_MOLES_TO_filt)
			continue

		var/specific_power = calculate_specific_power_gas(g, source, sink)/ATMOS_filt_EFFICIENCY
		specific_power_gas[g] = specific_power
		total_filtable_moles += source.gas[g]

	if (total_filtable_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1

	//now that we know the total amount of filtable gas, we can calculate the amount of power needed to scrub one mole of gas
	var/total_specific_power = 0		//the power required to remove one mole of filtable gas
	for (var/g in filting)
		var/ratio = source.gas[g]/total_filtable_moles //this converts the specific power per mole of pure gas to specific power per mole of scrubbed gas
		total_specific_power += specific_power_gas[g]*ratio

	//Figure out how much of each gas to filt
	if (isnull(total_transfer_moles))
		total_transfer_moles = total_filtable_moles
	else
		total_transfer_moles = min(total_transfer_moles, total_filtable_moles)
	...
	if (total_transfer_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	for (var/g in filting)
		...
		//filt gas in proportion to the mole ratio
		transfer_moles = min(transfer_moles, total_transfer_moles*(source.gas[g]/total_filtable_moles))

		//use update=0. All the filted gasses are supposed to be added simultaneously, so we update after the for loop.
...
//Generalized gas filting proc.
//filting is a bit different from scrubbing. Instead of selectively moving the targeted gas types from one gas mix to another, filting splits
...
//filting - A list of gasids to be filted. These gasses get moved to sink_filted, while the other gasses get moved to sink_clean.
//total_transfer_moles - Limits the amount of moles to input. The actual amount of gas filted may also be limited by available_power, if given.
//available_power - the maximum amount of power that may be used when filting gas. If null then the filting is not limited by power.
/proc/filt_gas(var/obj/machinery/M, var/list/filting, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_filted, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	var/total_filtable_moles = 0		//the total amount of filtable gas
	var/total_unfiltable_moles = 0	//the total amount of non-filtable gas
	var/list/specific_power_gas = list()	//the power required to remove one mole of pure gas, for each gas type
	for (var/g in source.gas)
		...
		if (g in filting)
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_filted)/ATMOS_filt_EFFICIENCY
			total_filtable_moles += source.gas[g]
		else
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_clean)/ATMOS_filt_EFFICIENCY
			total_unfiltable_moles += source.gas[g]
	...
	var/filted_power_used = 0		//power used to move filtable gas to sink_filted
	var/unfilted_power_used = 0	//power used to move unfiltable gas to sink_clean
	for (var/g in removed.gas)
		var/power_used = specific_power_gas[g]*removed.gas[g]

		if (g in filting)
			//use update=0. All the filted gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filted.adjust_gas_temp(g, removed.gas[g], removed.temperature, update=0)
			removed.adjust_gas(g, -removed.gas[g], update=0)
			filted_power_used += power_used
		else
			unfilted_power_used += power_used

	sink_filted.update_values()
	removed.update_values()

	sink_clean.merge(removed)

	return filted_power_used + unfilted_power_used

//For omni devices. Instead filting is an associative list mapping gasids to gas mixtures.
//I don't like the copypasta, but I decided to keep both versions of gas filting as filt_gas is slightly faster (doesn't create as many temporary lists, doesn't call update_values() as much)
//filt_gas can be removed and replaced with this proc if need be.
/proc/filt_gas_multi(var/obj/machinery/M, var/list/filting, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	for (var/g in source.gas)
		...
		if (g in filting)
			var/datum/gas_mixture/sink_filted = filting[g]
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_filted)/ATMOS_filt_EFFICIENCY
			total_filtable_moles += source.gas[g]
		else
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_clean)/ATMOS_filt_EFFICIENCY
	...
	var/list/filted_power_used = list()		//power used to move filtable gas to the filted gas mixes
	var/unfilted_power_used = 0	//power used to move unfiltable gas to sink_clean
	for (var/g in removed.gas)
		var/power_used = specific_power_gas[g]*removed.gas[g]
		...
		if (g in filting)
			var/datum/gas_mixture/sink_filted = filting[g]
			//use update=0. All the filted gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filted.adjust_gas_temp(g, removed.gas[g], removed.temperature, update=1)
			removed.adjust_gas(g, -removed.gas[g], update=0)
			if (power_used)
				filted_power_used[sink_filted] = power_used
		else
			unfilted_power_used += power_used

	removed.update_values()

	var/power_draw = unfilted_power_used
	for (var/datum/gas_mixture/sink_filted in filted_power_used)
		power_draw += filted_power_used[sink_filted]
...
/proc/mix_gas(var/obj/machinery/M, var/list/mix_sources, var/datum/gas_mixture/sink, var/total_transfer_moles = null, var/available_power = null)
	...
	for (var/datum/gas_mixture/source in mix_sources)
		if (source.total_moles < MINIMUM_MOLES_TO_filt)
		...
		source_specific_power[source] = calculate_specific_power(source, sink)*mix_ratio/ATMOS_filt_EFFICIENCY
		total_specific_power += source_specific_power[source]
		total_input_volume += source.volume
		total_input_moles += source.total_moles

	if (total_mixing_moles < MINIMUM_MOLES_TO_filt) //if we cant transfer enough gas just stop to avoid further processing
		return -1
