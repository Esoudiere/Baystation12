...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper/Topic(href,href_list)
	...
	if(filt.get("eject"))
		go_out()
	if(filt.get("view_stats"))
		chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(filt.get("inject"))
		inject_reagent(filt.getType("inject",/datum/reagent),filt.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/filt = new (href,href_list)
	if(filt.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(filt.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = filt.get("reagent_[i]")
		...
		return
	if(filt.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(filt.get("purge_reagent"))
		var/reagent = filt.get("purge_reagent")
		if(reagent)
			reagents.del_reagent(reagent)
		return
	if(filt.get("purge_all"))
